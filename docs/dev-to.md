# Dev.to features/pecularities that should be considered

## Accessibility suggestion

> Improve the accessibility of your post
> Consider changing "# New beginnings" to a level two heading by using "##" Learn more
> Consider changing "# Building the plane while flying it" to a level two heading by using "##" Learn more

## Tags

-   Can use only 4 tags
-   tags should not contain `-` symbol

## Front Matter

Leaving front matter in the beginning of the article bricks dev.to's editor. So we have to manually insert tags and set title
